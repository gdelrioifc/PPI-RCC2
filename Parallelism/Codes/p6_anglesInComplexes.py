...
for m in range(len(models)):
    ...
    for c in conditions:
        ...
        for q in range(samples):
            ...
            # This section calculates for real vectors of interacting proteins
            name = models[m]+'/size'+c+'/Interacting/'+input_name_str[m].split('_')[0]+'_complexes_'+c+'.csv'
            name_out = models[m]+'/size'+c+'/Interacting/AnglesAt_'+input_name_str[m].split('_')[0]+'_complexes_'+c+'.txt'

            # Uncomment this section to calculate for simulted vectors
            name = models[m]+'/size'+c+'/Interacting/sim_'+input_name_str[m].split('_')[0]+'_complexes_'+c+'_'+str(q)+'.csv'
            name_out = models[m]+'/size'+c+'/Interacting/AnglesAt_sim_'+input_name_str[m].split('_')[0]+'_complexes_'+c+'_'+str(q)+'.txt'
            ...
            average += len(parallels)/len(total)/samples

            input_file.close()
            output_file.close()

        print(models[m],c)
        print('paralelos (%):',100*average)
