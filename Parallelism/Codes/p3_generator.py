...
def massiveGenerator(directory,kind,case,elements,fln):
    # fln: sample number
    ...
    #print('Trying to read averges file')
...
for m in range(len(models)):
    for size in conditions:
        ...
        if models[m] == 'FullPDB':
            # This code is used if FullPDB has very different sample sizes
        ...
        for sample_number in range(samples):
            try:
                vector = massiveGenerator(main_dir,input_name_str[m],size,population,sample_number)
            except:
